...
class Node:
    ...
    def __repr__(self) -> str:
        return f"Node(name: {self.name()}, ub: {self.ub}, lb: {self.lb}, cost: {self.cost}, vars: {self.vars}, value: {self.zone_values})"
...
def add_arcs(nodes: Dict[str, Node], arcs: Dict[tuple, Arc], u: Node, v: Node):
    ...
    arc_configurations = {
        (NodeType.source, NodeType.plant): (1, 0),
        ...
        (NodeType.town, NodeType.sink): (u.ub, 0),
    }
...
def get_node(nodes, node_id: str, node_type: NodeType, ref_data: Dict[str, Any], **kwargs) -> Node:
    ...
    match node_type:
        ...
        case NodeType.lodge:
            ...
            assert ub and zone, "Lodge node requires 'ub' and 'zone' kwargs."
            zones = [zone]
        case NodeType.sink:
            ub = ref_data["max_capacity"]
...
def process_plant(
    ...
    add_arcs(nodes, arcs, nodes["source"], plant)


def process_town(
    nodes: Dict[str, Node], arcs: Dict[tuple, Arc], town: Node, ref_data: Dict[str, Any]
):
    """Add town node and arc to sink. Create lodge nodes for independent variable setup."""
    add_arcs(nodes, arcs, town, nodes["sink"])

    lodging_bonus = ref_data["lodging_bonus"]
    lodging_data = ref_data["lodging_data"][ref_data["town_to_root"][town.id]]
    ...
    lodge_node = get_node(
        ...
        NodeType.lodge,
        ref_data,
        ub=min(ref_data["waypoint_ub"], lodging_data["max_capacity"]),
        zone=town,
    )
    ...
    for ub, cost in lodges:
        lodge_node.vars[f"for_{ub}"] = {"lb": lb, "ub": ub, "cost": cost - prev_cost}
        prev_cost = cost
        lb = ub + 1
...
def generate_empire_data(config):
    ...
    lodgings = {k: v for k, v in nodes_dict.items() if v.isLodging}
    # Remove lodgings from nodes_dict
    for lodge in lodgings:
        nodes_dict.pop(lodge)

    graph_data: GraphData = {"V": nodes_dict, "E": arcs_dict, "towns": towns, "lodgings": lodgings}
    ...
    for arc in arcs_dict.values():
        zones = []
        for zone in set(arc.source.zones).intersection(set(arc.destination.zones)):
            # The only outflow when zone == town is to sink.
            if (arc.source.isTown and arc.destination.isWaypoint and arc.source == zone) or (
                arc.source.isTown and arc.destination.type is NodeType.sink and arc.source != zone
            ):
                continue
            zones.append(zone)
        arc.zones = zones
