...
from before_shutdown_data_gen import (
    NodeType as NT,
    Node,
    Arc,
...
def link_in_out_flows(prob: LpProblem, v: Node, in_arcs: List[Arc], out_arcs: List[Arc]):
    def get_fr_vars(v, arcs, var_key):
        return [var for a in arcs for k, var in a.vars.items() if k == var_key]
    ...
    for zone in v.zones:
        ...
        prob += lpSum(inflows) == lpSum(outflows), f"fBalance_{key}_at_{v.name()}"
    f = v.vars["f"]
...
def create_problem(config, G):
    ...
    for arc in G["E"].values():
        # Zone specific f ∈ ℕ₀ vars for each arc 0 <= f <= ub
        for zone in arc.zones:
            key = f"fz_{zone.id}"
            if arc.source.type in [NT.source, NT.plant] or arc.destination.type is NT.plant:
                arc.vars[key] = LpVariable(f"{key}_on_{arc.name()}", 0, cat="Binary")
            else:
                arc.vars[key] = LpVariable(f"{key}_on_{arc.name()}", 0, zone.ub, "Integer")
        ...
        if v.isPlant:
            for zone in v.zones:
                for arc in v.inbound_arcs:
                    values.append(v.zone_values[zone.id]["value"] * arc.vars[f"fz_{zone.id}"])
    prob += lpSum(values), "ObjectiveFunction"

    # Constraints

    # Link lodging f var to lodging x vars for lodging cost calculations
    lodge_cost_vars = []
    for lodge in G["lodgings"].values():
        arc = G["E"][(f"town_{lodge.id}", "sink")]
        f = arc.vars[f"fz_{lodge.id}"]
        for k, var in lodge.vars.items():
            x = LpVariable(f"x_{lodge.name()}_{k}", cat="Binary")
            prob += f - var["lb"] + 1 <= lodge.ub * x, f"xlb_{lodge.name()}_{k}"
            lodge_cost_vars.append((var["cost"], x))

    prob += (
        cost
        == (
            lpSum(v.cost * v.vars["x"] for v in G["V"].values())
            + lpSum(v[0] * v[1] for v in lodge_cost_vars)
        ),
        "TotalCost",
    )
    ...
    # Zone specific f⁻ == f⁺
    for v in G["V"].values():
        if v.type not in [NT.source, NT.sink]:
            link_in_out_flows(prob, v, v.inbound_arcs, v.outbound_arcs)

    # All zones f⁻sink == f⁺source
    link_in_out_flows(
        prob, G["V"]["sink"], G["V"]["sink"].inbound_arcs, G["V"]["source"].outbound_arcs
    )
...
def extract_solution(config, prob, graph_data):
    ...
    for k, v in solution_vars.items():
        if k.startswith("fz") and "_on_plant_" in k:
            tokens = k.split("_")
            zone, plant = (tokens[1], tokens[4])
            calculated_value += graph_data["V"][f"plant_{plant}"].zone_values[zone]["value"]
        if k.startswith("f_"):
            kname = k.replace("f_", "")
            if "_to_" in k:
                # An arc
...
def main(config):
    ...
    #  5 =>          0       0         0   0.00%   13953400.80399  13900824.11        0.38%        4      0      0      1220     1.2s   1.3s
    # 10 =>  L       0       0         0   0.00%   25014394.85418  24954483.03        0.24%     2046    151     61      2667     1.9s   2.0s
    # 20 =>          3       1         1  25.00%   49443164.26845  44334018.51       11.52%      366    127    135     39174    12.0s  11.9s
    # 30 =>        527      14       245  77.15%   64942970.52342  58540725          10.94%     2134    235   9919    308521    96.8s  97.8s
    # 50 =>       7463      27      3583  99.06%   87786260.36406  84340234.96001     4.09%     1429    388  10018     1957k   731.4s 707.0s
    ...
    for budget in [5, 10, 20, 30, 50]:
        ...
        config["solver"]["file_prefix"] = "BSS-A"
