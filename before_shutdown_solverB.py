...
from before_shutdown_data_genB import (
    NodeType as NT,
    Node,
    Arc,
...
def create_problem(config, G):
    ...
    for a in G["E"].values():
        ...
        for zone in set(a.source.zones).intersection(set(a.destination.zones)):
            if a.source == zone and a.destination.type is not NT.lodge:
                continue
            if a.source == zone and a.destination.type is NT.lodge:
                ub = a.destination.ub
            else:
                ub = a.ub if a.source.type in [NT.𝓢, NT.𝓣, NT.lodge] else zone.ub
    ...
    # A single lodge for each town.
    for town in G["towns"].values():
        vars = []
        for v in G["V"].values():
            if v.name().startswith(f"lodge_{town.id}_"):
                vars.append(v.vars["x"])
        prob += lpSum(vars) <= 1, f"Lodge_{town.id}"
...
def main(config):
    ...
    # 20 =>          3       1         1  25.00%   49443164.26845  44334018.51       11.52%      366    127    135     39174    12.0s
    # 30 =>        527      14       245  77.15%   64942970.52342  58540725          10.94%     2134    235   9919    308521    96.8s
    # 50 =>       7463      27      3583  99.06%   87786260.36406  84340234.96001     4.09%     1429    388  10018     1957k   731.4s
    ...
    for budget in [501]:
        ...
        config["solver"]["file_prefix"] = "BSS-C"
