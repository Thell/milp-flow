...
from before_shutdown_data_gen import generate_empire_data, GraphData, get_reference_data
...
def generate_graph(graph_data: GraphData, solution_data: Dict[str, Any]):
    ...
    for var_key in solution_data["solution_vars"].keys():
        ...
        u, v = None, None
        if "fz_" in var_key and "_on_" in var_key:
...
def process_solution_vars_file(solution_data):
    ...
    by_groups = {
        str(ref_data["townnames"]["name"][k]): v
...
def main():
    ...
    for filepath in input_files:
        if "BSS-A_mc50_lb0_tn4_nn5_wc25_gdefault_t25k_84340235.json" not in filepath:
            continue
