...
def link_in_out_by_region_highs(model: Highs, v: Node, in_arcs: list[Arc], out_arcs: list[Arc]) -> None:
    ...
    for region in v.regions:
        ...
        model.addConstr(model.qsum(inflows) == model.qsum(outflows), name=f"balance_{regionflow_key}_at_{v.name()}")
...
def create_model(config: dict, G: GraphData) -> Highs:
    ...
    for i, connect_set in enumerate(connect_sets):
        ...
        model.addConstr(model.qsum([G["V"][f"waypoint_{wp}"].vars["x"] for wp in connect_set]) >= len(connect_set) * x)
    model.addConstr(model.qsum(connect_vars) >= G["V"]["region_619"].vars["x"])

    return model


def optimize(data: dict, graph_data: GraphData) -> Highs:
